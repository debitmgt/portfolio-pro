      ...
      {plan === 'free' && (
        <div style={{ background: 'var(--accent-tint)', borderBottom: '1px solid var(--border)', padding: '8px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <span style={{ fontSize: 13, color: 'var(--text)' }}>
            Free plan Â· {holdings.length}/{FREE_LIMIT} holdings Â· Fundamentals &amp; Watchlist are Pro
          </span>
                ...
                <tr key={h.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{ color: atRisk ? 'var(--red)' : 'var(--muted)', fontWeight: atRisk ? 700 : 400 }}>
                      {h.trail_pct}%{stopPrice != null ? ` â†’ $${stopPrice.toFixed(2)}` : ''}
                      {atRisk && ' âš ï¸'}
                    </span>
                  </td>
                  <td style={{ padding: '12px 16px' }}>
  ...
  return (
    <ProTabShell title="Position Status" description="How each position has moved relative to your own cost basis and stop threshold  -  a personal status view, not a market call.">
      ...
      <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 20 }}>
        âš ï¸ These describe what's changed in a position  -  they are not buy, sell, or hold instructions. Always do your own research.
      </p>
    </ProTabShell>
  )
}
            ...
            {withWeights.sort((a, b) => b.weight - a.weight).map(h => (
              <div key={h.id}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--muted)' }}>{h.weight.toFixed(1)}% Â· ${h.value.toFixed(0)}</span>
                </div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr style={{ borderBottom: '1px solid var(--border)', textAlign: 'left' }}>
            <th style={{ padding: '8px 6px', fontWeight: 600, color: 'var(--muted)' }}>Symbol</th>
            <th style={{ padding: '8px 6px', fontWeight: 600, color: 'var(--muted)' }}>Beta</th>
          </tr>
        </thead>
        <tbody>
          {holdings.map(h => (
            <tr key={h.symbol} style={{ borderBottom: '1px solid var(--border)' }}>
              ...
              <td style={{ padding: '8px 6px' }}>{betaMap[h.symbol] != null ? betaMap[h.symbol]!.toFixed(2) : '\u2014'}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '16px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12 }}>
        ...
        {data?.name && <div style={{ fontSize: 12, color: 'var(--muted)' }}>{data.name}{data.industry ? ` Â· ${data.industry}` : ''}</div>}
      </div>
      {loading ? (
        <p style={{ color: 'var(--muted)', fontSize: 13 }}>Loading - </p>
      ) : error ? (
      ...
      <p style={{ fontSize: 11, color: 'var(--muted)', marginTop: 12 }}>
        Data from Finnhub Â· Full detail at <a href={`https://finance.yahoo.com/quote/${symbol}`} target="_blank" rel="noreferrer" style={{ color: 'var(--accent)' }}>Yahoo Finance â†—</a>
      </p>
    </div>
  )
}
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {items.map((item, i) => (
            ...
            >
              ...
              <div style={{ fontSize: 11.5, color: 'var(--muted)' }}>{item.source} Â· {timeAgo(item.datetime)}</div>
            </a>
          ))}
        </div>
      )}
      <p style={{ fontSize: 11, color: 'var(--muted)', marginTop: 12 }}>
        Headlines from Finnhub Â· Informational only, not commentary on whether to hold or sell.
      </p>
    </div>
  )
}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 24 }}>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '36px 32px', width: '100%', maxWidth: 500, position: 'relative' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: 14, right: 16, background: 'transparent', color: 'var(--muted)', fontSize: 22, padding: '0 6px', lineHeight: 1 }}>Ã—</button>
