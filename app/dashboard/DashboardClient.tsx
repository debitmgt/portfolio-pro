...
const ALL_TABS = ['Tracker', 'My Returns', 'Fundamentals', 'Dividends', 'Benchmark', 'News', 'Allocation View', 'Concentration', 'Charts', 'Watchlist']
...
// Counts from its previous value up (or down) to a new value whenever it
// changes — used for the Tracker summary cards so the dashboard feels alive
// on load and when prices refresh, not just a chart-page effect.
...
export default function DashboardClient({ userId, email, plan, initialHoldings }: Props) {
  ...
  useEffect(() => {
    setTabVisible(false)
    const id = requestAnimationFrame(() => setTabVisible(true))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg)' }}>
      ...
      {/* ── Header ── */}
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 24px', borderBottom: '1px solid rgba(255,255,255,0.12)', background: '#14161c', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
          ...
          <span style={{ fontWeight: 800, fontSize: 17, letterSpacing: '-0.3px', color: 'var(--accent)' }}>Ownfolio LLC</span>
          {priceLoading && <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.55)', marginLeft: 2 }}>↻</span>}
          <Link
            href="/disclaimer"
            title="Ownfolio LLC provides data and analytics, not personalized investment advice — see full disclaimer"
            style={{
              fontSize: 10.5, fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase',
              color: 'rgba(255,255,255,0.75)', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.18)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <span style={{
            ...
            background: plan === 'pro' ? 'var(--accent)' : 'rgba(255,255,255,0.1)',
            color: plan === 'pro' ? '#fff' : 'rgba(255,255,255,0.8)',
            letterSpacing: '0.05em',
          }}>{plan.toUpperCase()}</span>
          <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.75)' }}>{email}</span>
          {plan === 'free' && (
          ...
          {plan === 'pro' && (
            <button
              onClick={() => fetch('/api/stripe/portal').then(r => r.json()).then(d => { if (d.url) window.location.href = d.url })}
              style={{ fontSize: 13, background: 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.4)', borderRadius: 4, padding: '7px 14px' }}
            >
              Manage Plan
            </button>
          )}
      ...
      <nav style={{ display: 'flex', gap: 22, padding: '0 24px', borderBottom: '1px solid var(--border)', background: 'var(--bg)', overflowX: 'auto', flexShrink: 0 }}>
        {ALL_TABS.map(tab => {
          const locked = tab !== 'Tracker' && tab !== 'Dividends' && plan === 'free'
      ...
      {/* ── Main content ── */}
      <main style={{ flex: 1, overflow: 'auto', padding: 24, opacity: tabVisible ? 1 : 0, transform: tabVisible ? 'translateY(0)' : 'translateY(6px)', transition: 'opacity 0.2s ease, transform 0.2s ease' }}>
        ...
        {activeTab === 'My Returns' && (
          <LockedTabPreview locked={plan === 'free'} tabName="My Returns" onUpgrade={() => setShowUpgrade(true)}>
            <MyReturnsTab holdings={holdings} prices={prices} />
          </LockedTabPreview>
        )}

        {activeTab === 'Allocation View' && (
          <LockedTabPreview locked={plan === 'free'} tabName="Allocation View" onUpgrade={() => setShowUpgrade(true)}>
            <OptimizerTab holdings={holdings} prices={prices} totalValue={totalValue} />
          </LockedTabPreview>
        )}

        {activeTab === 'Concentration' && (
          <LockedTabPreview locked={plan === 'free'} tabName="Concentration" onUpgrade={() => setShowUpgrade(true)}>
            <ConcentrationTab holdings={holdings} prices={prices} totalValue={totalValue} />
          </LockedTabPreview>
        )}

        {activeTab === 'Charts' && (
          <LockedTabPreview locked={plan === 'free'} tabName="Charts" onUpgrade={() => setShowUpgrade(true)}>
            <ChartsTab holdings={holdings} prices={prices} />
          </LockedTabPreview>
        )}

        {activeTab === 'Benchmark' && (
          <LockedTabPreview locked={plan === 'free'} tabName="Benchmark" onUpgrade={() => setShowUpgrade(true)}>
            <BenchmarkTab holdings={holdings} prices={prices} />
          </LockedTabPreview>
        )}

        {activeTab === 'Dividends' && (
          <DividendsTab holdings={holdings} plan={plan} onUpgrade={() => setShowUpgrade(true)} />
        )}
        ...
        {activeTab === 'News' && (
          <LockedTabPreview locked={plan === 'free'} tabName="News" onUpgrade={() => setShowUpgrade(true)}>
...
function TrackerTab({ holdings, prices, plan, totalValue, totalCost, totalGain, totalPct, onAdd, onRemove, showAddForm, onCancelAdd, newSymbol, setNewSymbol, newShares, setNewShares, newCost, setNewCost, newTrail, setNewTrail, formError, saving, onSave, freeLimit, editingId, onStartEdit, onCancelEdit, onSaveEdit, editShares, setEditShares, editCost, setEditCost, editTrail, setEditTrail, editError, editSaving }: {
  ...
  return (
    <div>
      {/* Summary cards */}
      <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 20, color: 'var(--accent)' }}>Tracker</h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'Portfolio Value', raw: totalValue, format: (n: number) => `$${n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, color: 'var(--text)' },
          { label: 'Total Cost Basis', raw: totalCost, format: (n: number) => `$${n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, color: 'var(--text)' },
          { label: 'Total Gain / Loss', raw: totalGain, format: (n: number) => `${n >= 0 ? '+' : ''}$${n.toFixed(2)} (${totalPct.toFixed(2)}%)`, color: totalGain >= 0 ? 'var(--green)' : 'var(--red)' },
        ].map(({ label, raw, format, color }) => (
          <div key={label} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '16px 20px' }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 700, color }}><AnimatedNumber value={raw} format={format} /></div>
          </div>
        ))}
      </div>

      {/* Holdings table */}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'auto', maxHeight: '60vh', marginBottom: 20 }}>
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface)', position: 'sticky', top: 0, zIndex: 1 }}>
              {['Symbol', 'Shares', 'Cost / Share', 'Current Price', 'Value', 'Gain / Loss', 'Trail Stop', ''].map(h => (
                <th key={h} style={{ padding: '11px 16px', textAlign: 'left', color: 'var(--accent)', fontWeight: 500, fontSize: 12, whiteSpace: 'nowrap' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
              ...
              if (isEditing) {
                return (
                  <tr key={h.id} style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface-2)' }}>
                    <td style={{ padding: '12px 16px', fontWeight: 700, fontSize: 15 }}><span style={{ color: getSymbolColor(h.symbol) }}>{h.symbol}</span></td>
      ...
      {showAddForm ? (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: 24, maxWidth: 560 }}>
          <h3 style={{ marginBottom: 18, fontSize: 15, fontWeight: 600, color: 'var(--accent)' }}>Add Holding</h3>
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface)', position: 'sticky', top: 0, zIndex: 1 }}>
              {['#', 'Symbol', 'Current Price', 'Cost Basis', 'Your Return %', 'Market Value'].map(h => (
                <th key={h} style={{ padding: '11px 16px', textAlign: 'left', color: 'var(--accent)', fontWeight: 500, fontSize: 12 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
  ...
  return (
    <ProTabShell title="Allocation View" description="How your holdings compare to the targets you set. You choose the targets — this just shows the math.">
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '20px 24px', marginBottom: 20 }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 16, color: 'var(--accent)' }}>Portfolio Allocation</h3>
        {holdings.length === 0 ? <EmptyState /> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {withWeights.sort((a, b) => b.weight - a.weight).map(h => (
              <div key={h.id}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: getSymbolColor(h.symbol) }}>
                    {h.symbol}
                  </span>
      ...
      {(topHeavy.length > 0 || underweight.length > 0) && (
        <div style={{ display: 'grid', gap: 12 }}>
          <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--accent)' }}>Allocation Notes</h3>
          {topHeavy.map(h => (
...
function ConcentrationTab({ holdings, prices, totalValue }: { holdings: Holding[]; prices: PriceMap; totalValue: number }) {
  ...
  return (
    <ProTabShell title="Concentration" description="How much of your portfolio each holding represents — a fact, not a formula telling you what to buy.">
      {holdings.length > 0 && <DiversificationBreakdown withWeight={withWeight} />}

      <div style={{ display: 'grid', gap: 12, marginTop: holdings.length > 0 ? 28 : 0 }}>
        {holdings.length === 0 && <EmptyState />}
        {withWeight.map(h => (
          <div key={h.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '16px 20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 16, color: getSymbolColor(h.symbol) }}>
                  {h.symbol}
                </div>
...
// Same per-holding weights as above, grouped by Finnhub's industry and country
// classification (from /stock/profile2 — free tier, already used elsewhere)
// instead of by symbol. Same "fact, not a formula" framing as the rest of this
// tab. Each grouping gets both a bar list and a pie chart.
function DiversificationBreakdown({ withWeight }: { withWeight: Array<{ symbol: string; weight: number }> }) {
  const [industryMap, setIndustryMap] = useState<Record<string, string | null>>({})
  const [countryMap, setCountryMap] = useState<Record<string, string | null>>({})
    ...
    Promise.all(
      symbols.map(sym =>
        fetch(`/api/finnhub?type=fundamentals&symbol=${encodeURIComponent(sym)}`)
          ...
          .then((d: { industry: string | null; country: string | null } | null) => [sym, d?.industry ?? null, d?.country ?? null] as [string, string | null, string | null])
          .catch(() => [sym, null, null] as [string, string | null, string | null])
      )
    ).then(triples => {
      if (cancelled) return
      const iMap: Record<string, string | null> = {}
      const cMap: Record<string, string | null> = {}
      for (const [sym, industry, country] of triples) { iMap[sym] = industry; cMap[sym] = country }
      setIndustryMap(iMap)
      setCountryMap(cMap)
  ...
  const groupBy = (map: Record<string, string | null>, fallback: string) => {
    const totals: Record<string, number> = {}
    for (const h of withWeight) {
      const key = map[h.symbol] ?? fallback
      totals[key] = (totals[key] ?? 0) + h.weight
    }
    return Object.entries(totals).sort((a, b) => b[1] - a[1])
  }

  const byIndustry = groupBy(industryMap, 'Unclassified')
  const byCountry = groupBy(countryMap, 'Unclassified')
  const stillLoading = loading && Object.keys(industryMap).length === 0

  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 24 }}>
      <DiversificationSection
        title="By Industry"
        note="The same holdings above, grouped by industry classification instead of by symbol — sourced from public company data, not a sector call of our own."
        grouped={byIndustry}
        loading={stillLoading}
      />
      <DiversificationSection
        title="By Country"
        note="Grouped by each company's country of domicile, per public company data. Most funds and companies here are U.S.-based, so don't expect much spread if that's your portfolio."
        grouped={byCountry}
        loading={stillLoading}
      />
    </div>
...
const PIE_COLORS = ['var(--accent)', 'var(--green)', '#f59e0b', '#ec4899', '#14b8a6', '#8b5cf6', '#f97316', '#64748b', '#22c55e', '#eab308']

// Same color every time for a given symbol, regardless of sort order or
// which tab it's shown in — a simple string hash into the shared palette
// above, so e.g. AAPL is always the same color on Tracker, Charts, Watchlist, etc.
...
function DiversificationSection({ title, note, grouped, loading }: {
  title: string; note: string; grouped: Array<[string, number]>; loading: boolean
}) {
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: 20 }}>
      <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 4, color: 'var(--accent)' }}>{title}</h3>
      <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 16 }}>{note}</p>
      {loading ? (
        <p style={{ fontSize: 13, color: 'var(--muted)' }}>Loading…</p>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 24, alignItems: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {grouped.map(([label, weight], i) => (
              <div key={label}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                ...
                <div style={{ background: 'var(--border)', borderRadius: 4, height: 8, overflow: 'hidden' }}>
                  <div style={{ width: `${Math.min(weight, 100)}%`, height: '100%', background: PIE_COLORS[i % PIE_COLORS.length], borderRadius: 4 }} />
                </div>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <DiversificationPie grouped={grouped} />
          </div>
        </div>
      )}
    </div>
...
// Simple CSS conic-gradient pie chart — no chart library needed, and it's
// just a different view of the exact same numbers shown in the bars above.
function DiversificationPie({ grouped }: { grouped: Array<[string, number]> }) {
  const total = grouped.reduce((s, [, w]) => s + w, 0)
  if (total <= 0) return null

  let cursor = 0
  const stops: string[] = []
  grouped.forEach(([, weight], i) => {
  ...
  return (
    <div
      role="img"
      aria-label="Pie chart of portfolio weight distribution"
      style={{
        width: 160,
        height: 160,
        borderRadius: '50%',
    ...
    <ProTabShell title="Charts" description="Visual breakdown of your portfolio composition and performance.">
      {holdings.length === 0 ? <EmptyState /> : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: 20 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 16, color: 'var(--accent)' }}>Allocation by Symbol</h3>
            <AllocationBar holdings={holdings} prices={prices} totalValue={totalValue} />
          </div>

          {/* Gain/Loss bar chart */}
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: 20 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 16, color: 'var(--accent)' }}>Gain / Loss by Position</h3>
  ...
  return (
    <div>
      <div style={{ display: 'flex', height: 28, borderRadius: 6, overflow: 'hidden', marginBottom: 16 }}>
        {sorted.map((h) => (
          <div key={h.symbol} style={{ width: `${h.pct}%`, background: getSymbolColor(h.symbol), transition: 'width .3s' }} title={`${h.symbol}: ${h.pct.toFixed(1)}%`} />
        ))}
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {sorted.map((h) => (
          <div key={h.symbol} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
            <div style={{ width: 10, height: 10, borderRadius: 2, background: getSymbolColor(h.symbol), flexShrink: 0 }} />
            <span style={{ color: 'var(--muted)' }}><span style={{ color: getSymbolColor(h.symbol), fontWeight: 700 }}>{h.symbol}</span> {h.pct.toFixed(1)}%</span>
          </div>
        ))}
      </div>
    </div>
      ...
      {data.map(d => (
        <div key={d.symbol}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: getSymbolColor(d.symbol) }}>
              {d.symbol}
            </span>
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '16px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12 }}>
        <div style={{ fontWeight: 700, fontSize: 16, color: getSymbolColor(symbol) }}>
          {symbol}
        </div>
...
function NewsCard({ symbol }: { symbol: string }) {
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '16px 20px' }}>
      <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 12, color: getSymbolColor(symbol) }}>
        {symbol}
      </div>
      {loading ? (
        <p style={{ color: 'var(--muted)', fontSize: 13 }}>Loading…</p>
...
function StopLossTab({ holdings, prices }: { holdings: Holding[]; prices: PriceMap }) {
  ...
  return (
    <ProTabShell title="Drawdown Alerts" description="See when a holding is down from its trailing-stop level. Informational only — no action is implied or recommended.">
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'auto', maxHeight: '60vh', marginTop: triggered.length ? 16 : 0 }}>
          ...
          <tbody>
            ...
            {withStop.map(h => (
              <tr key={h.id} style={{ borderBottom: '1px solid var(--border)', background: h.triggered ? 'var(--red-tint)' : 'transparent' }}>
                <td style={{ padding: '12px 16px', fontWeight: 700 }}><span style={{ color: getSymbolColor(h.symbol) }}>{h.symbol}</span></td>
                <td style={{ padding: '12px 16px' }}>{h.price ? `$${h.price.toFixed(2)}` : '—'}</td>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {items.map(item => (
            <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px' }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: getSymbolColor(item.symbol) }}>{item.symbol}</span>
...
// ─── Benchmark Tab ────────────────────────────────────────────────────────────
// Compares your holdings' blended trailing price returns against SPY and QQQ
// (as S&P 500 / NASDAQ proxies) across several windows. This is a snapshot
// comparison built from the trailing-return fields Finnhub already returns
// for free (same fields used in the Fundamentals tab's Price Performance
// section) — not a continuous historical line chart, since that requires
// daily candle data not included on the current Finnhub plan. Each holding's
// return is weighted by its current market value; a symbol missing a given
// window's data is simply excluded from that window's average, not treated
// as zero.
const BENCHMARK_SYMBOLS = { sp500: 'SPY', nasdaq: 'QQQ' } as const
const RETURN_WINDOWS: Array<{ key: 'fiveDay' | 'monthToDate' | 'thirteenWeek' | 'twentySixWeek' | 'fiftyTwoWeek' | 'yearToDate'; label: string }> = [
  { key: 'fiveDay', label: '1 Week' },
  { key: 'monthToDate', label: 'Month to Date' },
  { key: 'thirteenWeek', label: '~3 Months' },
  { key: 'twentySixWeek', label: '~6 Months' },
  { key: 'fiftyTwoWeek', label: '1 Year' },
...
function BenchmarkTab({ holdings, prices }: { holdings: Holding[]; prices: PriceMap }) {
  const [perf, setPerf] = useState<Record<string, PricePerformance | null>>({})
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const symbols = [...new Set([...holdings.map(h => h.symbol), BENCHMARK_SYMBOLS.sp500, BENCHMARK_SYMBOLS.nasdaq])]
    let cancelled = false

    Promise.all(
      symbols.map(symbol =>
        fetch(`/api/finnhub?type=fundamentals&symbol=${symbol}`)
          .then(r => (r.ok ? r.json() : null))
          .then(res => [symbol, res?.pricePerformance ?? null] as const)
          .catch(() => [symbol, null] as const)
      )
    ).then(results => {
      if (cancelled) return
      const next: Record<string, PricePerformance | null> = {}
      results.forEach(([symbol, p]) => { next[symbol] = p })
      setPerf(next)
      setLoading(false)
    })
  ...
  const portfolioReturn = (windowKey: typeof RETURN_WINDOWS[number]['key']): number | null => {
    let weightedSum = 0
    let weightUsed = 0
    for (const h of holdings) {
      const p = perf[h.symbol]
      const r = p?.[windowKey]
      if (r == null) continue
      const value = (prices[h.symbol] ?? h.cost_basis) * h.shares
      weightedSum += r * value
  ...
  const pctColor = (v: number | null) => (v == null ? 'var(--muted)' : v >= 0 ? 'var(--green)' : 'var(--red)')

  return (
    <ProTabShell title="Benchmark" description="How your holdings' blended trailing returns compare to the S&P 500 (SPY) and NASDAQ (QQQ) across a few time windows — a snapshot comparison, not a running historical chart, and not a recommendation to change anything.">
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'auto', maxHeight: '60vh' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14, minWidth: 560 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface)', position: 'sticky', top: 0, zIndex: 1 }}>
              {['Window', 'Your Portfolio', 'S&P 500 (SPY)', 'NASDAQ (QQQ)', 'vs. S&P 500'].map(h => (
                <th key={h} style={{ padding: '11px 16px', textAlign: 'left', color: 'var(--accent)', fontWeight: 500, fontSize: 12, whiteSpace: 'nowrap' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {RETURN_WINDOWS.map(w => {
              const yours = loading ? null : portfolioReturn(w.key)
              const sp500 = perf[BENCHMARK_SYMBOLS.sp500]?.[w.key] ?? null
              const nasdaq = perf[BENCHMARK_SYMBOLS.nasdaq]?.[w.key] ?? null
              const diff = yours != null && sp500 != null ? yours - sp500 : null
              return (
                <tr key={w.key} style={{ borderBottom: '1px solid var(--border)' }}>
                  <td style={{ padding: '12px 16px', fontWeight: 600 }}>{w.label}</td>
                  <td style={{ padding: '12px 16px', fontWeight: 700, color: pctColor(yours) }}>{loading ? '…' : fmtPct(yours)}</td>
                  <td style={{ padding: '12px 16px', color: pctColor(sp500) }}>{loading ? '…' : fmtPct(sp500)}</td>
                  <td style={{ padding: '12px 16px', color: pctColor(nasdaq) }}>{loading ? '…' : fmtPct(nasdaq)}</td>
                  <td style={{ padding: '12px 16px', fontWeight: 600, color: pctColor(diff) }}>{loading ? '…' : diff == null ? '—' : `${diff >= 0 ? '+' : ''}${diff.toFixed(1)} pts`}</td>
                </tr>
              )
            })}
          </tbody>
      ...
      <p style={{ fontSize: 11.5, color: 'var(--muted)', marginTop: 12 }}>
        Your portfolio figures are a blend of each holding's own trailing return, weighted by its current value in your account — not a time-weighted return that accounts for when you actually bought in. Total portfolio value used for weighting: ${totalValue.toLocaleString('en-US', { maximumFractionDigits: 0 })}.
      </p>
    </ProTabShell>
  )
}
...
// Estimated annual income, yield, and 5-year dividend growth rate — all
// sourced from the same fundamentals call Finnhub already includes on the
// free tier. (An earlier version also showed actual payment history/dates
// via /stock/dividend, but that endpoint isn't included on the current
// Finnhub plan, so it's been removed rather than show "Unavailable" to
// every user. Revisit if the Finnhub plan is ever upgraded.)
interface DividendFundamentals {
  perShareAnnual: number | null
  yieldTTM: number | null
  growthRate5Y: number | null
}
interface DividendSymbolData {
  fundamentals: DividendFundamentals | null
  loading: boolean
}

function DividendsTab({ holdings, plan, onUpgrade }: { holdings: Holding[]; plan: Plan; onUpgrade: () => void }) {
  const [data, setData] = useState<Record<string, DividendSymbolData>>({})

  useEffect(() => {
    const symbols = [...new Set(holdings.map(h => h.symbol))]
    symbols.forEach(symbol => {
      if (data[symbol]) return   // already fetched (or in flight)
      setData(prev => ({ ...prev, [symbol]: { fundamentals: null, loading: true } }))
      ...
      fetch(`/api/finnhub?type=fundamentals&symbol=${symbol}`)
        ...
        .then(fundRes => {
          setData(prev => ({
            ...prev,
            [symbol]: {
              fundamentals: fundRes?.dividends
                ? { perShareAnnual: fundRes.dividends.perShareAnnual, yieldTTM: fundRes.dividends.yieldTTM, growthRate5Y: fundRes.dividends.growthRate5Y }
                : null,
              loading: false,
            },
          }))
        }).catch(() => {
          setData(prev => ({ ...prev, [symbol]: { fundamentals: null, loading: false } }))
        })
    })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [holdings])
  ...
  const totalEstAnnualIncome = holdings.reduce((sum, h) => {
    const perShare = data[h.symbol]?.fundamentals?.perShareAnnual
    return sum + (perShare ? perShare * h.shares : 0)
  }, 0)

  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 4, color: 'var(--accent)' }}>Dividends</h2>
        <p style={{ color: 'var(--muted)', fontSize: 14 }}>Estimated income for your holdings.</p>
      </div>

      {/* ── Free: summary + per-holding table ── */}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '16px 20px', marginBottom: 20 }}>
        <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 6, fontWeight: 500 }}>Estimated Annual Dividend Income</div>
        <div style={{ fontSize: 22, fontWeight: 700 }}>
          ${totalEstAnnualIncome.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </div>
        <div style={{ fontSize: 11.5, color: 'var(--muted)', marginTop: 4 }}>
          Based on current per-share annual dividend rates × your shares held — not a guarantee of future payments.
        </div>
      </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'auto', maxHeight: '60vh', marginBottom: 20 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14, minWidth: 480 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface)', position: 'sticky', top: 0, zIndex: 1 }}>
              {['Symbol', 'Div / Share (annual)', 'Yield', 'Est. Annual Income'].map(h => (
                <th key={h} style={{ padding: '11px 16px', textAlign: 'left', color: 'var(--accent)', fontWeight: 500, fontSize: 12, whiteSpace: 'nowrap' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {holdings.map(h => {
              ...
              const perShare = d?.fundamentals?.perShareAnnual ?? null
              const yieldPct = d?.fundamentals?.yieldTTM ?? null
              const estIncome = perShare != null ? perShare * h.shares : null
              return (
                <tr key={h.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  <td style={{ padding: '12px 16px', fontWeight: 700 }}><span style={{ color: getSymbolColor(h.symbol) }}>{h.symbol}</span></td>
                  <td style={{ padding: '12px 16px' }}>{d?.loading ? '…' : perShare != null ? `$${perShare.toFixed(2)}` : '—'}</td>
                  <td style={{ padding: '12px 16px' }}>{d?.loading ? '…' : yieldPct != null ? `${yieldPct.toFixed(2)}%` : '—'}</td>
                  <td style={{ padding: '12px 16px', fontWeight: 600 }}>{estIncome != null ? `$${estIncome.toFixed(2)}` : '—'}</td>
                </tr>
              )
            })}
          </tbody>
      ...
      {/* ── Pro: 5-year dividend growth rate ── */}
      <InlineProLock locked={plan === 'free'} label="Dividend Growth Rates" onUpgrade={onUpgrade}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'auto', maxHeight: '60vh' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14, minWidth: 320 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface)', position: 'sticky', top: 0, zIndex: 1 }}>
                {['Symbol', '5-Year Dividend Growth'].map(h => (
                  <th key={h} style={{ padding: '11px 16px', textAlign: 'left', color: 'var(--accent)', fontWeight: 500, fontSize: 12, whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {holdings.map(h => {
                const growth5Y = data[h.symbol]?.fundamentals?.growthRate5Y ?? null
                return (
                  <tr key={h.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '12px 16px', fontWeight: 700 }}><span style={{ color: getSymbolColor(h.symbol) }}>{h.symbol}</span></td>
                    <td style={{ padding: '12px 16px' }}>{growth5Y != null ? `${growth5Y.toFixed(1)}%` : '—'}</td>
                  </tr>
                )
              })}
            </tbody>
...
// Inline (not whole-tab) Pro lock — shows a blurred preview of the section
// itself with an upgrade card, for tabs like Dividends that have a genuine
// free tier plus a Pro-only section within the same page.
function InlineProLock({ locked, label, onUpgrade, children }: { locked: boolean; label: string; onUpgrade: () => void; children: React.ReactNode }) {
  if (!locked) return <>{children}</>
  return (
    <div style={{ position: 'relative', minHeight: 200 }}>
      <div aria-hidden="true" style={{ filter: 'blur(6px)', opacity: 0.85, pointerEvents: 'none', userSelect: 'none' }}>
        {children}
      </div>
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, transparent 0%, var(--bg) 65%)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', paddingBottom: 20 }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '18px 26px', textAlign: 'center', maxWidth: 340, boxShadow: '0 8px 28px rgba(0,0,0,.14)' }}>
          <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 6 }}>🔒 {label} is a Pro feature</div>
          <p style={{ fontSize: 12.5, color: 'var(--muted)', marginBottom: 12, lineHeight: 1.5 }}>
            Upgrade to see dividend growth trends for every holding.
          </p>
          <button className="btn-primary" onClick={onUpgrade} style={{ fontSize: 13, padding: '8px 18px', borderRadius: 6, border: 'none', cursor: 'pointer' }}>
            Upgrade to Pro →
          </button>
        </div>
      </div>
...
function ProTabShell({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 4, color: 'var(--accent)' }}>{title}</h2>
