...
export async function sendNewSignupAlert(email: string): Promise<void> {
  try {
    ...
    const { error } = await resend.emails.send({
      from: fromAddress(),
      to: ALERT_TO,
      subject: `[Ownfolio] New free signup: ${email}`,
      html:
        `<p>New free-tier signup at ${new Date().toISOString()}:</p>` +
        `<p><strong>${escapeHtml(email)}</strong></p>`,
    })
    if (error) console.error('[alerts] Resend rejected new-signup alert email:', error)
  } catch (err) {
    console.error('[alerts] Failed to send new-signup alert email:', err)
  }
}
