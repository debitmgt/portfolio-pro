...
import type { MonthlyRanking, CapTier, NewsletterEditorial } from '@/lib/supabase/types'
...
function baseLayout(opts: {
  ...
  const { preheader, title, intro, editorial, sections, methodologyNote, unsubscribeUrl } = opts

  const sectionsHtml = sections.map(renderSectionTable).join('')
...
// ─── Free: three Top 25s (large/mid/small cap) ───────────────────────────────
export function renderMultiTierTop25Email(opts: {
  ...
  const { periodLabel, byTier, editorial, unsubscribeToken } = opts
      ...
      <table role="presentation" width="100%" style="max-width:480px;background:#14161c;border:1px solid #2a2d36;border-radius:8px;">
        <tr><td style="padding:24px;">
          ...
          <p style="margin:0 0 20px 0;color:#b6bac4;font-size:13px;line-height:1.6;">
            Click below to confirm you'd like the monthly Ownfolio Top 25 — three ranked lists (large, mid, and small cap) of the 25 highest trailing 1-year returns from our tracked universe, sent once a month. Informational only, not financial advice.
          </p>
...
export function renderWatchlistDigestEmail(opts: {
  ...
  const { periodLabel, allRankings, watchlistSymbols, editorial, unsubscribeToken } = opts
  const monthTitle = periodTitle(periodLabel)
